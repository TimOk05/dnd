...
function formatNpcBlocks(txt, forcedName = '') {
    ...
    if (trait && /служит|академи|обучает|преподает|мастерская|мешок|инструменты|станок|разбирает|носит|мечтает|стать|известным|советником|влияния|события|работает|рынке|призвание|собирать|слухи|истории|рубит|мясо/i.test(trait)) {
    ...
    if (trait && /высокий|низкий|стройный|полный|волосы|глаза|лицо|одежда|длинные|короткие|светлые|темные|крепкий|мужчина|плечи|руки|шрамы|фартук|хвост/i.test(trait)) {
        if (!appear) appear = trait;
        trait = '';
    }
    
    // Если в описании черты характера - переносим
    if (desc && /черты характера|прямолинейный|наблюдательный|грубоватым|юмор|харизматичный|проницательный|ответственный|надменный|артистичный|дипломатичный|преданный|терпеливый|внимательный|мечтательный|общительный|находчивый|рассеянный/i.test(desc.toLowerCase())) {
        if (!trait || trait === '-') {
            trait = desc;
            desc = '';
        }
    ...
    if (!appear || appear === '-') {
        ...
        for (let line of lines) {
            let lineLower = line.toLowerCase();
            if (/высокий|низкий|стройный|полный|волосы|глаза|лицо|одежда|длинные|короткие|светлые|темные|красивые|острые|широкие|узкие|борода|усы|морщины|крепкий|мужчина|плечи|руки|шрамы|фартук|хвост/i.test(lineLower) && line.length > 10 && line.length < 200) {
                appear = line;
                break;
            }
        }
    ...
    if (desc) {
        ...
        for (let line of descLines) {
            ...
            if (/харизматичный|проницательный|ответственный|надменный|артистичный|дипломатичный|преданный|терпеливый|внимательный|мечтательный|общительный|находчивый|рассеянный/i.test(lineLower)) {
    ...
    if (other && /черты характера|любознательный|обаятельный|нетерпеливый|преданный|наивный/i.test(other.toLowerCase())) {
        if (!trait || trait === '-') {
            trait = other;
            other = '';
        }
    ...
    if (desc && desc !== '-') {
        out += `<div class='npc-col-block'><span style='font-size:1.2em;'>📜</span> <b>Описание</b>${firstSentence(desc)}</div>`;
    }
